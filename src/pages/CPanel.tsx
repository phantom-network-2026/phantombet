  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        {buckets.map(b => (
          <Button key={b} variant={selectedBucket === b ? "gold" : "outline"} size="sm"
      ...
      {currentPath && (
        <button onClick={() => setCurrentPath(prev => { const p = prev.split("/"); p.pop(); return p.join("/"); })}
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <p className="text-muted-foreground text-sm">Loading...</p> : rows.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          <Database className="h-12 w-12 mx-auto mb-3 opacity-30" /><p className="text-sm">No data</p>
        </div>
      ) : (
...
function MaintenancePanel() {
  ...
  const fetchData = async () => {
    ...
    if (maint.data?.value) {
      const v = maint.data.value as any;
      setMaintenance(v.enabled || false);
      setMaintenanceMsg(v.message || "");
    }
    if (ann.data?.value) {
      const v = ann.data.value as any;
      setAnnouncementActive(v.active || false);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">When ON, non-admin users see a maintenance page</p>
            </div>
          </div>
      ...
      <div className="rounded-lg bg-card border border-border p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-bold text-sm">Site Announcement</p>
              <p className="text-xs text-muted-foreground">Show a banner across the site for all users</p>
            </div>
          </div>
        ...
        <div>
          ...
          <Textarea value={announcement} onChange={e => setAnnouncement(e.target.value)} className="bg-background border-border text-xs mt-1" rows={2} placeholder="🎉 Welcome bonus! Get 100% on your first deposit..." />
...
function AnalyticsDashboard() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      setStats({
        ...
        activeFriendships: friendships.count || 0,
      });
      setLoading(false);
    };
    fetch();
  ...
  const cards = [
    { label: "Total Users", value: stats.totalUsers, icon: Users, color: "text-casino-gold" },
    { label: "Online Now", value: stats.onlineUsers, icon: Activity, color: "text-green-400" },
    { label: "Total Balance", value: `$${stats.totalBalance.toFixed(2)}`, icon: BarChart3, color: "text-casino-green" },
    { label: "Transactions", value: stats.totalTransactions, icon: Clock, color: "text-blue-400" },
    { label: "Games", value: stats.totalGames, icon: Table, color: "text-purple-400" },
    { label: "Messages", value: stats.totalMessages, icon: Megaphone, color: "text-pink-400" },
    { label: "Active Bans", value: stats.totalBans, icon: Ban, color: "text-destructive" },
    { label: "Prize Spins", value: stats.totalSpins, icon: RefreshCw, color: "text-casino-gold" },
    { label: "New This Week", value: stats.recentSignups, icon: Bell, color: "text-green-400" },
    { label: "Friendships", value: stats.activeFriendships, icon: Users, color: "text-blue-400" },
  ];
...
function ErrorLogs() {
  ...
  const fetchLogs = async (type: string) => {
    setLoading(true);
    setLogType(type as any);
    let data: any[] = [];
    if (type === "moderation") {
      const res = await supabase.from("moderation_log").select("*").order("created_at", { ascending: false }).limit(100);
      data = (res.data || []).map((l: any) => ({
        timestamp: l.created_at, level: "INFO", message: `${l.action_type} on ${l.target_user_id?.slice(0, 8)}...`,
        detail: l.reason || "No reason", source: l.game_room || "Global",
      }));
    } else if (type === "transactions") {
      const res = await supabase.from("transactions").select("*").order("created_at", { ascending: false }).limit(100);
      data = (res.data || []).map((t: any) => ({
        timestamp: t.created_at, level: t.amount < 0 ? "WARN" : "INFO",
        message: `${t.type}: $${Math.abs(t.amount).toFixed(2)}`,
        detail: t.description || "—", source: t.user_id?.slice(0, 8) + "...",
      }));
    } else if (type === "chat") {
      const res = await supabase.from("game_chat").select("*").order("created_at", { ascending: false }).limit(100);
      data = (res.data || []).map((c: any) => ({
        timestamp: c.created_at, level: "INFO",
        message: `${c.username || "Unknown"}: ${c.content?.slice(0, 80)}`,
        detail: c.game_room, source: c.user_id?.slice(0, 8) + "...",
      }));
    }
    setLogs(data);
    setLoading(false);
      ...
      <div className="container max-w-5xl py-6 px-4">
        ...
        <Tabs defaultValue="analytics" className="space-y-4">
          <TabsList className="w-full bg-card border border-border flex-wrap h-auto gap-1 p-1">
            ...
            <TabsTrigger value="files" className="text-xs gap-1"><FolderOpen className="h-3 w-3" /> Files</TabsTrigger>
            <TabsTrigger value="database" className="text-xs gap-1"><Database className="h-3 w-3" /> Database</TabsTrigger>
