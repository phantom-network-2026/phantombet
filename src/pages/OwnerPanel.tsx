...
export default function OwnerPanel() {
  ...
  const [adjustType, setAdjustType] = useState<"bonus" | "real">("bonus");
  ...
  const fetchPanelToggles = async () => {
    try {
      ...
      console.error("Failed to fetch panel toggles", err);
    }
  ...
  const handlePanelToggle = async (key: string, checked: boolean) => {
    ...
    try {
      ...
      if (existing) {
        const { error } = await supabase.from("site_settings").update({ value: updated as any }).eq("key", "panel_visibility");
        if (error) throw error;
      } else {
        const { error } = await supabase.from("site_settings").insert({ key: "panel_visibility", value: updated as any });
        if (error) throw error;
      }
      ...
      // Revert
      setPanelToggles(prev => ({ ...prev, [key]: !checked }));
    }
  };
    ...
    if (profiles) {
      const usersWithRoles = profiles.map((p) => {
        ...
        return {
          ...
          real_balance: Number((p as any).real_balance) || 0,
  ...
  const handleAdjustBalance = async (userId: string, amount: number) => {
    const balField = adjustType === "real" ? "real_balance" : "balance";
    const user = users.find((u) => u.user_id === userId);
    if (!user) return;
    const currentBal = adjustType === "real" ? user.real_balance : user.balance;

    const { error: txError } = await supabase.from("transactions").insert({
      user_id: userId, amount, type: adjustType === "real" ? "real_adjustment" : "adjustment",
      description: adjustDescription || `Owner ${adjustType} balance adjustment: ${amount > 0 ? "+" : ""}${amount}`,
    });
    if (txError) { toast.error("Failed to create transaction"); return; }
    
    const { error } = await supabase.from("profiles").update({ [balField]: currentBal + amount } as any).eq("user_id", userId);
    if (error) { toast.error("Failed to update balance"); return; }
    toast.success(`${adjustType === "real" ? "Real" : "Bonus"} balance ${amount > 0 ? "added" : "deducted"}: $${Math.abs(amount).toFixed(2)}`);
  ...
  const handleEditUser = (userId: string) => {
    ...
    setEditEmail(""); setEditPassword(""); setProtectionCode("");
  ...
  const handleSaveUserDetails = async (userId: string) => {
    ...
    const isTargetOwner = targetUser?.roles.includes("owner");
    ...
    if (editEmail.trim() || editPassword.trim()) {
      const { data, error } = await supabase.functions.invoke("admin-update-user", {
        body: {
          ...
          ...(isTargetOwner && { protection_code: protectionCode }),
        },
      });
    ...
    setEditUserId(null); setEditEmail(""); setEditPassword(""); setEditUsername(""); setProtectionCode("");
    setSaving(false);
    fetchUsers();
  };
        ...
        <div className="space-y-3">
          ...
          {loadingUsers ? (
            ...
            users.map((user) => {
              ...
              return (
                <div key={user.user_id} className={`rounded-xl bg-card border p-4 ${isTargetOwner ? "border-red-500/30" : "border-border"}`}>
                  <div className="flex items-center justify-between mb-2 gap-2">
                    ...
                    <div className="text-right shrink-0 ml-2">
                      <p className="font-display font-bold text-casino-gold text-sm">${user.balance.toFixed(2)}</p>
                      <p className="text-[10px] text-casino-green">Real: ${user.real_balance.toFixed(2)}</p>
                    </div>
                  </div>

                  <div className="flex gap-2 flex-wrap">
                    <Button variant="outline" size="sm" onClick={() => { setAdjustUserId(adjustUserId === user.user_id ? null : user.user_id); setAdjustType("bonus"); }}>
                  ...
                  {/* Delete Confirmation */}
                  {deleteConfirmId === user.user_id && !isTargetOwner && (
                  ...
                  {/* Edit User Details — Owner has full access with protection code for owner account */}
                  {editUserId === user.user_id && (
                    <div className="mt-3 p-3 rounded-lg bg-secondary space-y-2 animate-slide-up">
                      ...
                      {isTargetOwner && (editEmail.trim() || editPassword.trim()) && (
                        <div className="rounded-lg bg-red-500/10 border border-red-500/30 p-2">
                          <Label className="text-xs text-red-400">🔒 Protection Code Required</Label>
                          <Input type="password" value={protectionCode} onChange={(e) => setProtectionCode(e.target.value)} placeholder="Enter protection code" className="bg-background border-red-500/30 mt-1" />
                        </div>
                      )}
                  ...
                  {/* Adjust Balance Form — Owner can adjust both bonus and real */}
                  {adjustUserId === user.user_id && (
                    <div className="mt-3 p-3 rounded-lg bg-secondary space-y-2 animate-slide-up">
                      <div className="flex gap-2">
                        <Button variant={adjustType === "bonus" ? "gold" : "outline"} size="sm" onClick={() => setAdjustType("bonus")} className="text-xs">
                          Bonus Balance
                        </Button>
                        <Button variant={adjustType === "real" ? "gold" : "outline"} size="sm" onClick={() => setAdjustType("real")} className="text-xs">
                          <Wallet className="h-3 w-3 mr-1" /> Real Balance
                        </Button>
                      </div>
                      <div className="flex gap-2">
                      ...
                      <div>
                        ...
                        <Input value={adjustDescription} onChange={(e) => setAdjustDescription(e.target.value)} placeholder="Owner adjustment" className="bg-background border-border" />
                      </div>
                      <div className="flex gap-2">
