 // --- Splash Logic ---
        ...
        let wallet = parseInt(localStorage.getItem('bomb_premium_wallet')) || 1000;
        let historyData = JSON.parse(localStorage.getItem('bomb_premium_history')) || [];
        ...
        function updateUI() {
            elWallet.textContent = wallet;
            localStorage.setItem('bomb_premium_wallet', wallet);
            localStorage.setItem('bomb_premium_history', JSON.stringify(historyData));
        }
        ...
        function adjustBet(n) {
            ...
            let val = parseInt(elBetInput.value) + n;
            if(val < 10) val = 10;
            elBetInput.value = val;
            playSfx(sndClick);
        }
        ...
        function confirmBet() {
            ...
            const amt = parseInt(elBetInput.value);
            if(amt > wallet) {
        ...
        function decideResult() {
            ...
            if(win) {
                const winAmt = Math.floor(lockedBet * PAYOUT_MULT);
                wallet += winAmt;
                addHistory('WIN', winAmt);
                updateUI();

                elStatus.innerHTML = `<span class="text-green-500 font-black"><i class="fas fa-check-circle mr-2"></i>Success +${winAmt}</span>`;
                ...
                return `
                    <div class="history-item">
                        ...
                        <div class="flex items-center gap-4">
                            <span class="${colorClass} font-orbitron font-black text-xs">${item.amount > 0 ? '+' : ''}${item.amount}</span>
        ...
        function openAd() {
            ...
            const int = setInterval(() => {
                ...
                if(t <= 0) {
                    clearInterval(int);
                    btn.disabled = false;
                    btn.classList.remove('bg-white/5', 'text-gray-600');
                    btn.classList.add('bg-indigo-600', 'text-white');
                    btn.textContent = "CLAIM +100 CREDITS";
                }
            }, 1000);
        }
        ...
        function closeAd() {
            wallet += 100;
            addHistory('DEPOSIT', 100);
            updateUI();
            document.getElementById('ad-overlay').classList.replace('flex', 'hidden');
            showToast("Credits Transmitted", "green");
        }
