// 23.10.2025 - fix find win (linbehavior)
// 14.10.2025 - fix fefresh total bet text (after hold use)
// 16.09.2025 - added slotstatistic
// 22.12.2024
class Lamp
{
...
class SlotSymbol
{
    ...
    changeSymbol(icon, blurTexture)
    {
        ...
        if(this.scene [this.anim_sheet + '_animation'] == null) this.scene [this.anim_sheet + '_animation'] = this.scene.anims.create({
            key: this.anim_sheet + 'anim',
            frames: this.scene.anims.generateFrameNumbers(this.anim_sheet),
            frameRate: this.frameRate,
            repeat: -1
...
class Reel{
    ...
    _create() 
    {
       ...
       // this.scene['reel' + this.reelNumber].depth = -3;
        ...
        this.shape.fillRect(this.posX - this.symbolSizeY * 0.6, this.windowPosY + this.symbolSizeY * 0.5,  this.symbolSizeY * 1.2, -this.windowsCount * this.symbolSizeY * 1.0);
        ...
        for(var si = 0; si < this.symbOrder.length; si++)
        {
            ...
            symbSprite.setMask(this.gMask);                          // apply mask
    ...
    scaleMaskAndOffset(xScale, yScale, xOffset, yOffset)
    {
        this.shape.setScale(xScale, yScale);
        this.shape.x += xOffset;
    ...
    findWin(payTable)
    {
        ...
        payTable.forEach((item)=>
        {
            ...
            if (winTemp != null)
            {
                ...
                {
                    if(this.win.pay < winTemp.pay || this.win.freeSpins < winTemp.freeSpins) // 23.10.2025
                    {
                        this.win = winTemp;
                    }
                }      
        ...
        if (this.useJackpot && this.jackpotWinSymbols != null && this.jackpotWinSymbols.length > 0)
        {
            ...
            this.jackpotWinSymbols.forEach((s)=>{
                ...
                pAj.add((callBack) =>
                {
                    ...
                    new SimpleTweenFloat(this, 0, 1, 3000, (p, dp) =>{ }, ()=>{s.showAnim(false); callBack();} ); // just delay action
                });
            });
...
class SlotControls
{
    ...
    refreshBetLines()
    {
        ...
        if (this.totalBetSumText != null) this.totalBetSumText.text = this.getTotalBet();
...
class StatisticData
{
    constructor(bet, winCoins, isFreeSpin,  lineWins, scatterWin, jpWin)
    {
        this.bet = bet;
        ...
        if(lineWins !== null)
        {
            ...
            {
                lineWins.forEach ((item)=>{this.lineWins.push(new WinData(item.win.symbols, item.win.freeSpins, item.win.pay));})         
            }
        }

        if (scatterWin != null) this.scatterWin = new WinData(scatterWin.symbols, scatterWin.freeSpins, scatterWin.pay);
        if (jpWin != null) this.jpWin = new WinData(jpWin.symbols, jpWin.freeSpins, jpWin.pay);      
    }
    toString()
    {
        return "Bet: " + this.bet + "; Win Coins: " + this.winCoins;
    }
}

class SlotStatistic
{
    constructor(slotPlayer)
    {
        this.maxCount = 10000;
        this.statisticData = [];
        this.initBalance = 0;
    ...
    add(sData)
    {
        ...
        {
            this.init = true;
            this.initBalance = this.slotPlayer.coins;
        }
        this.statisticData.push(sData);
        // if (this.statisticData.length > this.maxCount) this.statisticData.RemoveAt(0);
    }
    ...
    printStatistic()
    {
        ...
        var sumPay = 0;
        var spins = 0;
        if(this.statisticData!=null && this.statisticData.length > 0)
        {
            spins = this.statisticData.length;
            this.statisticData.forEach((item)=>{sumBet = (item.isFreeSpin) ? sumBet : sumBet + item.bet; sumPay += item.winCoins;});
        }
        var payOut = sumPay / sumBet * 100.0;
        // var payOut = (this.slotPlayer.coins - this.initBalance + sumBet) / sumBet * 100.0; // full payout with mini games
        console.log("Statistic, balance :" + this.slotPlayer.coins + " ; sum bet: " + sumBet + " ;sum payout: " + sumPay + " ;spins: " + spins +" ; payout %: " + payOut);
    }
}
