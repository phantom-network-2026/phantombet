...
class SlotGame extends Phaser.Scene{
    ...
    preload(){
        ...
        this.load.spritesheet("coinspin", "png/CoinSheet.png", { frameWidth: 100, frameHeight: 100});
    ...
    create(){   
        ...
        this.endWinShowEvent_1 = new MKEvent();
        this.endWinShowEvent_2 = new MKEvent();
        ...
        // this.coinParticles = this.add.particles('coinspin').setDepth(2000); // on top of all objects - old Phaser 3.55
        this.coinParticles = this.add.particles(this.centerX, -100, 'coinspin', {       // new Phaser 3.88.2
            frame: 0,
            quantity: 3,
            frequency: 200,
            angle: { min: -30, max: 30 },
        ...
        this.slotStatistic = new SlotStatistic(this.slotPlayer);
        ...
        // 10)
        if(slotConfig.lastCreate) slotConfig.lastCreate(this);
        ...
        // 11a) tests
        // this.showWinCoinsMessage(20, 20000); // small message
        ...
        */ 
        ...
        // var wgPU =  this.guiController.showPopUp(slotConfig.createWheelGamePUHandler_6);
    ...
    runSlot()
    {
        ...
        sA.add((callBack) =>{
            ...
            this.endWinSearchEvent.events.forEach((eW)=>{if (eW != null && eW.action != null) eW.action.call(eW.context, anyWin);}); 
            callBack();
            }, this);  // search spin result

        sA.add((callBack) =>{
            anyWin = this.winController.hasAnyWinn() || this.sideWinFlag;       // check this.sideWinFlag
            this.sideWinFlag = false;                                           // reset this.sideWinFlag
            ...
            if(anyWin)
            {
                ...
                this.slotStatistic.add(new StatisticData(this.slotControls.getTotalBet(), summCoins, this.isFreeSpin, this.winController.winLines, this.winController.scatterWin, this.winController.jackPotWin));
            }
            else
            {
                this.isCascadeSpin = false;
                this.slotStatistic.add(new StatisticData(this.slotControls.getTotalBet(), 0, this.isFreeSpin, null, null, null));
            }
    ...
    *winShowC(completeCallBack)
    {
        ...
        //3a ------ any win show event, perhaps a message or minigame -------
        ...
        //3c0 -----------------calc coins, perhaps a message or minigame -------------------
        ...
        //3c1 ----------- calc free spins, perhaps a message or minigame----------------
        ...
        // 3d1 -------- add levelprogress, perhaps a message or minigame --------------
        ...
        // additional win events, minigames, messages
        ...
        this.endWinShowEvent_1.invoke();
        while(this.miniGame !== null || !this.guiController.hasNoPopUp())
        {
            yield null;
        }

        this.endWinShowEvent_2.invoke();
        while(this.miniGame !== null || !this.guiController.hasNoPopUp())
        {
            yield null;
        }
        ...
        // this.slotStatistic.printStatistic();
        completeCallBack();
    ...
    *loseShowC(completeCallBack)
     ...
     {
         var bwPU = this.guiController.showPopUp(slotConfig.createMegaWinPUHandler);
    ...
    // show coins particles; for Phaser 3.88.2
   showCoins(show)
   {
...
function spinReels(scene, reels, _slotConfig, completeCallback){
    ...
    var simulation = (scene.slotControls.simulateSpins) ? scene.slotControls.simulateSpins.getSimulation() : null;
    reels.forEach((r)=>{
        pA.add((callBack)=>
        {
            ...
            var rand = (simulation != null && simulation [ri] >= 0) ? simulation [ri] : r.getRandomOrderPosition();
